...
const offerTemplateFactory = (offerIds, type) => getArrayFromType(type).map((offer) => {
...
const photosTemplateFactory = (destination) => destination.pictures.map((pic) =>
  `<img class="event__photo" src="${pic.src}" alt="Event photo"></img>`
).join('');

const citiesListTemplateFactory = (cities) => cities.map((city) =>
  `<option value=${city}></option>`
).join('');

const pointMenuFactory = (point) => {
  ...
  return `<form class="event event--edit" action="#" method="post">
	<header class="event__header">
	  ...
	  <div class="event__field-group  event__field-group--destination">
		...
		<datalist id="destination-list-1">
		  ${citiesListTemplateFactory(CITIES)}
		</datalist>
	  </div>
    ...
    <div class="event_-available-offers">
      ${offerTemplateFactory(point.offers, point.type)}
    </div>
	  </section>
    ...
    <div class="event__photos-container">
      <div class="event__photos-tape">
        ${photosTemplateFactory(destination)}
      </div>
    </div>
	  </section>
	</section>
...
class CreationFormView extends AbstractView {
  ...
  get template() {
    return pointMenuFactory(this.point);
  }
}
